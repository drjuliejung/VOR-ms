...
#7    lots      medium     
#8    lots     none
clutchcolors<- c("cyan1", "cadetblue2",  "dodgerblue1", "blue2", "navyblue")
...
plot.daily <- ggplot(Daily.df, aes(x=Age, y=CorrectedAvg, group=as.factor(as.character(Age)))) +
  ...
  geom_jitter(data=stage3, aes(x=as.numeric(Age)+0.2, colour=stagecolors[1]), size=3,  position=position_jitterdodge(jitter.width=.3, jitter.height=0, dodge.width=0.75, seed=NA)) +
  geom_jitter(data=stage4, aes(x=as.numeric(Age)+0.2, colour=stagecolors[2]), size=3, position=position_jitterdodge(jitter.width=.3, jitter.height=0, dodge.width=0.75, seed=NA)) +
  geom_jitter(data=stage5, aes(x=as.numeric(Age)+0.2, colour=stagecolors[3]), size=3, position=position_jitterdodge(jitter.width=.3, jitter.height=0, dodge.width=0.75, seed=NA)) +
  geom_jitter(data=stage6, aes(x=as.numeric(Age)+0.2, colour=stagecolors[4]), size=3,  position=position_jitterdodge(jitter.width=.3, jitter.height=0, dodge.width=0.75, seed=NA)) +
    geom_jitter(data=stage7, aes(x=as.numeric(Age)+0.2, colour=stagecolors[5]), size=3,  position=position_jitterdodge(jitter.width=.3, jitter.height=0, dodge.width=0.75, seed=NA)) +
    geom_jitter(data=stage8, aes(x=as.numeric(Age)+0.2, colour=stagecolors[6]), size=3,  position=position_jitterdodge(jitter.width=.3, jitter.height=0, dodge.width=0.75, seed=NA)) +
  ...
  theme(legend.background = element_rect(fill="white",
                                  ...
                                  colour ="black")) + 
  annotate("text",x=3,y=15,label="a", size=10)+
  annotate("text",x=4,y=35,label="b", size=10)+
  annotate("text",x=5,y=33,label="b", size=10)+
  annotate("text",x=6,y=40,label="b", size=10)
...
plot.sibs<- ggplot(SubsetSibshipsSonia.df, aes(x=Age, y=CorrectedAvg, colour=Clutch)) +
  geom_point(data=c101, aes(x=as.numeric(Age), colour=clutchcolors[1]), size=3, position=position_jitterdodge(jitter.width=.1, jitter.height=.1, dodge.width=0.75, seed=NA)) +
  geom_point(data=c102, aes(x=as.numeric(Age), colour=clutchcolors[2]), size=3, position=position_jitterdodge(jitter.width=.1, jitter.height=.1, dodge.width=0.75, seed=NA)) +
  geom_point(data=c104, aes(x=as.numeric(Age), colour=clutchcolors[3]), size=3, position=position_jitterdodge(jitter.width=.1, jitter.height=.1, dodge.width=0.75, seed=NA)) +
  geom_point(data=c105, aes(x=as.numeric(Age), colour=clutchcolors[4]), size=3, position=position_jitterdodge(jitter.width=.1, jitter.height=.1, dodge.width=0.75, seed=NA)) +
  geom_point(data=c106, aes(x=as.numeric(Age), colour=clutchcolors[5]), size=3, position=position_jitterdodge(jitter.width=.1, jitter.height=.1, dodge.width=0.75, seed=NA)) +
  theme_cowplot(font_size = 18, line_size = 1) +
  labs(x = "\nDevelopmental age (days)", 
       y = "VOR amplitude (°)\n") +
  scale_color_identity(name="Clutch", 
  ...
  theme(legend.background = element_rect(fill="white",
                                         size=0.5, linetype="solid", 
                                         colour ="black")) + 
  scale_x_continuous(limits=c(3.75, 5.75), 
  ...
  theme(axis.title.y=element_text(size=18, colour = "black")) +
  annotate("text",x=3.75,y=7,label="a", size=10)+
  annotate("text",x=4,y=13,label="a", size=10)+
  annotate("text",x=4.25,y=30,label="b", size=10)+
  annotate("text",x=4.5,y=45,label="c", size=10)+
  annotate("text",x=4.75,y=15,label="d", size=10)+
  annotate("text",x=5.75,y=7,label="d", size=10)
...
```{r}
plot.sibs<- ggplot(SubsetSibshipsSonia.df, aes(x=Age, y=CorrectedAvg, colour=Clutch)) +
  geom_point(data=c101, colour="#FC0008", size=3) +
  geom_point(data=c102, colour="#21FF05", size=3) +
  geom_point(data=c104, colour="#0000FF", size=3) +
  geom_point(data=c105, colour="#FC8008", size=3) +
  geom_point(data=c106, colour="#800080", size=3) +
  theme_cowplot(font_size = 18, line_size = 1) +
  labs(x = "\nDevelopmental age (days)", 
       y = "VOR amplitude (°)\n") +
  theme(legend.justification = c(0,1), legend.position = c(0.05,1)) +
  theme(legend.background = element_rect(fill="white",
                                         ...
                                         colour ="black")) + 
  scale_color_identity(name="Clutch", 
                       breaks=c("#FC0008", "#21FF05", "#0000FF", "#FC8008", "#c800080"), 
                       labels=c(101, 102, 104, 105, 106), 
                       guide="legend") +
  scale_x_continuous(limits=c(3.75, 5.75), 
                     breaks = c(4, 4.5, 5, 5.5), 
...
ggsave("~/Desktop/Figure4.pdf", 
 plot = Figure4, # or give ggplot object name as in myPlot,
 width = 18, height = 21, 
...
```{r}
plot.stageGLM<-ggplot(tactile, aes(x=as.numeric(as.character(Developmental.Stage)), y=as.numeric(as.character((Response)), shape=as.factor(Response)))) +
...
```{r}
png("~/Desktop/Figure7.png", width = 18, height = 21, units = 'cm', res = 300)
grid.arrange (Figure7) # Make plot
...
Figure8 <- ggplot(tactile, aes(x=CorrectedAvg, y=as.numeric(as.character(Latency.to.Hatch.in.Minutes)))) +
  geom_smooth(method = "lm", se=TRUE, color="Black", linetype="dashed") +
  geom_point(data=st3.H, position=position_jitter(height=0.4, width=0.4), aes(colour = stagecolors[1]), size=3) +
  geom_point(data=st4.H, position=position_jitter(height=0.4, width=0.4), aes(colour = stagecolors[2]), size=3) +
  geom_point(data=st5.H, position=position_jitter(height=0.4, width=0.4), aes(colour = stagecolors[3]), size=3) +
  geom_point(data=st6.H, position=position_jitter(height=0.4, width=0.4), aes(colour = stagecolors[4]), size=3) +
  geom_point(data=st7.H, position=position_jitter(height=0.4, width=0.4), aes(colour = stagecolors[5]), size=3) +
  theme_cowplot(font_size = 14, line_size = 1) +
  labs(x = "\nVOR amplitude (°)", 
  ...
  theme(legend.background = element_rect(fill="white",
                                         ...
                                         colour ="black")) +
  scale_color_identity(name="Stage", 
                       breaks=c(stagecolors[1], stagecolors[2], stagecolors[3], stagecolors[4], stagecolors[5]), 
                       labels=c(3, 4, 5, 6, 7), 
                       guide="legend") +
  theme(axis.text.y=element_text(size=14, colour= "black")) +
  theme(axis.text.x=element_text(size=14, colour= "black")) +
...
```{r}
png("~/Desktop/VOR_ms_figures/Figure8.png", width = 18, height = 15, units = 'cm', res = 300)
grid.arrange (Figure8) # Make plot
dev.off()
```

```{r}
ggsave("~/Desktop/VOR_ms_figures/Figure8.pdf", 
