...
```{r}
library(xlsx);library(clinfun);library(car); library(MASS);library(multcomp);library(pscl);library(psych);library(lme4); library("stargazer");library("knitr");library("dplyr")
...
In clutches with partial vibration cued hatching, hatched embryos did have greater VOR than unhatched (hypoxia hatched) siblings. 
...
LT20 <- subset(tactile, CorrectedAvg < 20) #73
LT20_hat <- subset(LT20, Response == "1") #28
LT20_nohat <- subset(LT20, Response == "0") #45
#propH = 28/73 = 0.3835

LT15 <- subset(tactile, CorrectedAvg < 15) #59
LT15_hat <- subset(LT15, Response == "1") #18
LT15_nohat <- subset(LT15, Response == "0") #41
#propH = 18/59 = 0.3051

LT12 <- subset(tactile, CorrectedAvg < 12) #59
LT12_hat <- subset(LT12, Response == "1") #18
LT12_nohat <- subset(LT12, Response == "0") #41
#propH = 18/59 = 0.3051


LT10 <- subset(tactile, CorrectedAvg < 10) #51
LT10_hat <- subset(LT10, Response == "1") #12
LT10_nohat <- subset(LT10, Response == "0") #39
#propH = 12/51 = 0.2353

LT1 <- subset(tactile, CorrectedAvg < 1) #31
LT1_hat <- subset(LT1, Response == "1") #7
LT1_nohat <- subset(LT1, Response == "0") #24
#propH = 7/31 = 0.2258
```
...
```{r}
plot.LateralLine<-ggplot(tactile, aes(x=Developmental.Stage, y=CorrectedAvg, shape=as.factor(tactile$Response))) +
  geom_jitter(size=2, position=position_jitter(width=0.2, height=3))+
...
```{r}
ggsave("~/Desktop/Figure10.pdf", 
 plot = plot.LateralLine, # or give ggplot object name as in myPlot,
 width = 18, height = 15, 
 units = "cm", # other options c("in", "cm", "mm"), 
 dpi = 300)
dev.off()
...
Make a variable with 3 categories: 
1. NoHatch - no hatch (NH; black fill, pch = 19)
2. HatchNoVOR - 7 individuals that hatched with no VOR. (H; red fill, pch=1)
3. Hatch - other individuals that hatched but have VOR (H; no/white fill, pch=1)

```{r}
tactile$STAGExVOR[tactile$Response ==0] <- "NoHatch"
tactile$STAGExVOR[tactile$Response ==1 & tactile$CorrectedAvg==0] <- "HatchNoVOR"
tactile$STAGExVOR[tactile$Response ==1 & tactile$CorrectedAvg>0] <- "Hatch"

str(tactile)
```


Label the individuals that hatched with below threshold VOR. 

```{r}
plot.LateralLine<-ggplot(tactile, aes(x=Developmental.Stage, y=CorrectedAvg, shape=as.factor(tactile$STAGExVOR))) +
  geom_jitter(data=seven, pch=21, size=3, position=position_jitter(width=0.25, height=3)) + 
  geom_jitter(data=others, pch=19, size=3, position=position_jitter(width=0.25)) +
  geom_jitter(data=nohat, pch=1, size=3,  position=position_jitter(width=0.25)) +
  scale_y_continuous(limits=c(-3, 45),
                     breaks=c(0, 15, 30, 45),
                     labels=c("0", "15", "30", "45"))+
  scale_x_continuous(limits=c(1.5, 7.5),
  ...
  scale_shape_manual(name=NULL, 
                     values=c(19, 1),
                     labels=c("No hatch", "Hatch - yes VOR", "Hatch - no VOR")) +
  labs(y = "VOR amplitude (°) \n", 
       x = "\n Developmental stage")+
  theme_cowplot(font_size = 18, line_size = 1) +
  theme(axis.text.y=element_text(size=18, colour= "black")) +
